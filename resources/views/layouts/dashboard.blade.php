...
<body class="font-sans antialiased bg-slate-100 text-slate-800 h-screen overflow-hidden" x-data>
    <div class="h-full flex">
        <div
            class="hidden lg:block fixed inset-y-0 left-0 z-30 transition-all duration-300"
        ...
        >
            ...
            <div class="relative z-50 w-72 max-w-[85vw] h-full shadow-2xl">
                @include('layouts.partials.sidebar', ['forceExpanded' => true])
            </div>
        </div>

        <div
            class="flex-1 flex flex-col min-w-0 h-full min-h-0 transition-all duration-300"
            :class="$store.sidebar.collapsed ? 'lg:ml-[4.5rem]' : 'lg:ml-64'"
        >
